\documentclass[11pt, a4paper]{article}
\usepackage[utf8]{inputenc}
\usepackage{graphicx}
\usepackage{blindtext}
\usepackage{booktabs} % For \toprule, \midrule and \bottomrule
\usepackage{pgfplotstable} % Generates table from .csv
\usepackage{fancyhdr}

% change default font family from serif to sans-serif
\renewcommand{\familydefault}{\sfdefault}

% change ToC, figure and table section titles to german equivalents
\renewcommand{\contentsname}{Inhaltsverzeichnis}
\renewcommand{\listfigurename}{Figuren- \& Abbildungsverzeichnis}
\renewcommand{\listtablename}{Tabellenverzeichnis}

\pagestyle{fancy}
\fancyhf{}
\rhead{John Doe}
\lhead{Projekttitel}  % \leftmark
\chead{}
\rfoot{}
\lfoot{}
\cfoot{}

\newcommand{\frontmatter}{
  \clearpage
  \rfoot{}
}

\newcommand{\mainmatter}{
  \clearpage
  \rfoot{Seite \thepage\ von \pageref{LastPageOfFrontMatter}}
}

\newcommand{\backmatter}{
  \clearpage
  \rfoot{Seite \thepage\ von \pageref{LastPageOfBackMatter}}
}

\title{Projekttitel}
\date{2019-03-05}
\author{Projektdokumentation\\
       \\
       John Doe\\
       Awesome Denki - SuperElektrik GmbH\\
}

\begin{document}
  \pagenumbering{gobble}
  \maketitle
  \newpage

  \tableofcontents
  \newpage

  \pagenumbering{arabic}
  \mainmatter
  \section{Definitionsphase}
    \input{content/ch01.tex}
    \newpage

  \section{Planungsphase}
    \input{content/ch02.tex}
    \newpage

  \section{Implementierungsphase}
    \input{content/ch03.tex}
    \newpage

  \section{Abschlussphase}
    \input{content/ch04.tex}
    \label{LastPageOfFrontMatter}~
    \newpage

  \frontmatter
  \pagenumbering{gobble}
  \begin{appendix}
    \listoffigures
    \listoftables
  \end{appendix}
  \newpage

  \backmatter
  \section{Anhang: Figuren \& Tabellen}
    \pagenumbering{Roman}
    \subsection{Fragenkatalog zum Kundengespraech}
      \input{tables/fragenkatalog.tex}
      \newpage

    \subsection{Projektstrukturplan}
      \begin{figure}[h!]
        \makebox[\textwidth]{\includegraphics[width=1.4\linewidth]{fig/Projektstrukturplan.pdf}}
        \caption{Projektstrukturplan}
        \label{fig:psp}
      \end{figure}
      \newpage

    \subsection{Terminplan}
      \input{tables/zeitplan.tex}
      \newpage

    \subsection{Kostenplan}
      \input{tables/kostenplan.tex}
      \newpage

    \subsection{Qualitaetssicherungsplan}
      \input{tables/qualitaetssicherungsplan.tex}
      \newpage

    \subsection{Use-Case Diagramm}
      \begin{figure}[h!]
        \makebox[\textwidth]{\includegraphics[width=1.2\linewidth]{fig/use_case_diagram.pdf}}
        \caption{Use Case Diagram}
        \label{fig:usecasediagram}
      \end{figure}
      \newpage

    \subsection{Klassendiagramm}
      \begin{figure}[h!]
        \makebox[\textwidth]{\includegraphics[width=1.3\linewidth]{fig/UML_Classdiagram.pdf}}
        \caption{Klassendiagramm}
        \label{fig:classdiagram}
      \end{figure}
      \newpage

    \subsection{Entity Relationship Diagram, Datenbank}
      \begin{figure}[h!]
        \makebox[\textwidth]{\includegraphics[width=1.3\linewidth]{fig/ERD_JeffersonMiddleSchool.pdf}}
        \caption{Entity Relationship Diagram, Datenbank}
        \label{fig:datadiagram}
      \end{figure}
      \newpage

    \subsection{Zeitlicher Soll-Ist Vergleich}
      \input{tables/zeitlichersollistvergleich.tex}
      \newpage

    \subsection{Fachlicher Soll-Ist Vergleich}
      \input{tables/fachlichersollistvergleich.tex}
      \label{LastPageOfBackMatter}~
      \newpage

\end{document}
